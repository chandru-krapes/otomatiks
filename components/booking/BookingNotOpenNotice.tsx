...
/**
 * Temporary stand-in for the real checkout flow, in content form only — no
 * page or dialog chrome. Ticket booking opens the next Saturday from
 * whenever this is viewed. `CheckoutPage` (the actual checkout
 * implementation) is untouched and still wired up at `app/checkout/page.tsx`
 * — swap it back in for whichever shell renders this once bookings go live.
 * See flow.pdf follow-up: "for now" gate on checkout.
 *
 * Rendered by two shells: `CheckoutComingSoon` (full page, at `/checkout`
 * itself — the fallback for anyone who lands there directly) and
 * `BookingNotOpenDialog` (the in-page dialog opened from the cart drawer's
 * "Continue Booking" button — the primary path now).
 */
...
export default function BookingNotOpenNotice({
  event,
  onDismiss,
}: {
  event: Event;
  /** Present only when rendered inside a dialog — swaps "Back to tickets" /
   * "Return to event home" (page-navigation language) for a single "Got it"
   * that just closes the dialog instead of leaving the page. */
  onDismiss?: () => void;
}) {
  ...
  return (
    <div className="route-transition relative z-10 mx-auto flex w-full max-w-xl flex-col items-center text-center">
      ...
      <p className="mt-3 max-w-md text-sm leading-relaxed text-muted sm:text-base">
        We&rsquo;re putting the final touches on booking for {event.title}. Booking begins
      </p>
